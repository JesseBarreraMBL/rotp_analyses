...
merged_deidentified_long <- read.csv("merged_long_deidentified_treated.csv")

```

# Examine correlations

## CO mindset and primary outcomes
...
ggpairs(merged_deidentified_long_t1, c(42, 35:41, 50:53), progress = FALSE)

# T7

merged_deidentified_long_t3 <- merged_deidentified_long %>% 
  filter(timepoint == 3) 

ggpairs(merged_deidentified_long_t3, c(42, 35:41, 50:53), progress = FALSE)

# T8

merged_deidentified_long_t4 <- merged_deidentified_long %>% 
  filter(timepoint == 4) 

ggpairs(merged_deidentified_long_t4, c(42, 35:41, 50:53), progress = FALSE)


```


## CO mindset and secondary outcomes

```{r}

# T5

ggpairs(merged_deidentified_long_t1, c(42:49), progress = FALSE)

# T7

ggpairs(merged_deidentified_long_t3, c(42:49), progress = FALSE)

# T8

ggpairs(merged_deidentified_long_t4, c(42:49), progress = FALSE)


```

CO mindset and mediators/moderators

```{r}

# T5

ggpairs(merged_deidentified_long_t1, c(42, 54:57, 77:78), progress = FALSE)

# T7

ggpairs(merged_deidentified_long_t3, c(42, 54:56, 77:78), progress = FALSE)

# T8

ggpairs(merged_deidentified_long_t4, c(42, 54:57, 92, 97:101, 106), progress = FALSE)
...
## CO Mindset - Mixed Models

```{r}


# Mixed model with categorical timepoint
...
### Political Party x PHQ-Depression

```{r}

# Make new column with political_party as a binary variable
...
# Make a separate dataframe for each value of the binary political party variable

dems_simplified <- political_simplified %>% 
  filter(political_categorical == "Dem")

non_dems_simplified <- political_simplified %>% 
  filter(political_categorical == "Not Dem")

# Run and visualize mixed models for Dems

depression_dems.mm <- lmer(phq_depression ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(depression_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = phq_depression, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Depression Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 0:6, labels = 0:6) 
theme_minimal()

# Run and visualize mixed models for non-Dems

depression_non_dems.mm <- lmer(phq_depression ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(depression_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = phq_depression, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Depression Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 0:6, labels = 0:6) 
theme_minimal()
...
### Political Party x CO Mindset

```{r}

# Run and visualize mixed model for Dems

com_dems.mm <- lmer(co_mindset ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

summ(com_dems.mm)

sjPlot::tab_model(com_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = co_mindset, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of CO Mindset Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 1:7, labels = 1:7) 
theme_minimal()


# Run and visualize mixed model for non-Dems

com_non_dems.mm <- lmer(co_mindset ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

summ(com_non_dems.mm)

sjPlot::tab_model(com_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = co_mindset, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of CO Mindset Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 1:7, labels = 1:7) 
theme_minimal()
...
### Political x PTGI-Full

```{r}

# Run and visualize mixed models for Dems

ptgi_full_dems.mm <- lmer(ptgi_full ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

summ(ptgi_full_dems.mm)

sjPlot::tab_model(ptgi_full_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = ptgi_full, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI-Full Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 1:105) 
theme_minimal()

# Run and visualize mixed models for non-Dems

ptgi_full_non_dems.mm <- lmer(ptgi_full ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

summ(ptgi_full_non_dems.mm)

sjPlot::tab_model(ptgi_full_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = ptgi_full, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI-Full Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PHQ-Depression", limits = 1:105) 
theme_minimal()
...
### Political x PTGI-Relationships

```{r}

# Run and visualize mixed models for Dems

ptgi_relationships_dems.mm <- lmer(ptgi_relationships ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(ptgi_relationships_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = ptgi_relationships, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (Relationships) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-Relationships", limits = 0:40, labels = 0:40) +
theme_minimal()

# Run and visualize mixed models for non-Dems

ptgi_relationships_non_dems.mm <- lmer(ptgi_relationships ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(ptgi_relationships_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = ptgi_relationships, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (Relationships) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-Relationships", limits = 0:40, labels = 0:40) +
theme_minimal()
...
## Political x PTGI-Possibilities

```{r}

# Run and visualize mixed models for Dems

ptgi_possibilities_dems.mm <- lmer(ptgi_possibilities ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(ptgi_possibilities_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = ptgi_possibilities, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (possibilities) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:3, labels = 1:3) +
scale_y_discrete("PTGI-possibilities", limits = 0:30, labels = 0:30) +
theme_minimal()

# Run and visualize mixed models for non-Dems

ptgi_possibilities_non_dems.mm <- lmer(ptgi_possibilities ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(ptgi_possibilities_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = ptgi_possibilities, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (possibilities) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-possibilities", limits = 0:30, labels = 0:30) +
theme_minimal()
...
ptgi_appreciation_dems.mm <- lmer(ptgi_appreciation ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(ptgi_appreciation_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = ptgi_appreciation, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (appreciation) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:3, labels = 1:3) +
scale_y_discrete("PTGI-appreciation", limits = 0:30, labels = 0:30) +
theme_minimal()

# Run and visualize mixed models for non-Dems

ptgi_appreciation_non_dems.mm <- lmer(ptgi_appreciation ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(ptgi_appreciation_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = ptgi_appreciation, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (appreciation) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-appreciation", limits = 0:30, labels = 0:30) +
theme_minimal()
...
### Political x PTGI-Spiritual

```{r}

# Run and visualize mixed models for Dems

ptgi_spiritual_dems.mm <- lmer(ptgi_spiritual ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(ptgi_spiritual_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = ptgi_spiritual, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (spiritual) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:3, labels = 1:3) +
scale_y_discrete("PTGI-spiritual", limits = 0:30, labels = 0:30) +
theme_minimal()

# Run and visualize mixed models for non-Dems

ptgi_spiritual_non_dems.mm <- lmer(ptgi_spiritual ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(ptgi_spiritual_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = ptgi_spiritual, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of PTGI (spiritual) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 0:30, labels = 0:30) +
theme_minimal()
...
### Political x Meaning (Presence)

```{r}

# Run and visualize mixed models for Dems

meaning_presence_dems.mm <- lmer(meaning_presence ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(meaning_presence_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = meaning_presence, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Meaning (Presence) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:14, labels = 1:14) +
theme_minimal()

# Run and visualize mixed models for non-Dems

meaning_presence_non_dems.mm <- lmer(meaning_presence ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(meaning_presence_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = meaning_presence, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Meaning (Presence) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:14, labels = 1:14) +
theme_minimal()
...
### Political x Meaning (Search)

```{r}

# Run and visualize mixed models for Dems

meaning_search_dems.mm <- lmer(meaning_search ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(meaning_search_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = meaning_search, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Meaning (meaning_search) Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:28, labels = 1:28) +
theme_minimal()

# Run and visualize mixed models for non-Dems

meaning_search_non_dems.mm <- lmer(meaning_search ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(meaning_search_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = meaning_search, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Meaning (meaning_search) Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:28, labels = 1:28) +
theme_minimal()
...
resilience_dems.mm <- lmer(resilience ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = dems_simplified)

sjPlot::tab_model(resilience_dems.mm)

ggplot(dems_simplified, aes(x = timepoint, y = resilience, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Resilience Across Time x Condition (Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:28, labels = 1:28) +
theme_minimal()
...
resilience_non_dems.mm <- lmer(resilience ~ condition * as.factor(timepoint) + (1 | participant_id), 
                   data = non_dems_simplified)

sjPlot::tab_model(resilience_non_dems.mm)

ggplot(non_dems_simplified, aes(x = timepoint, y = resilience, color = condition)) +
geom_point(alpha = 0.3, size = 1, position = position_jitter(width = 0.1, height = 0.2)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Linear Mixed Effects Model of Resilience Across Time x Condition (Non-Democrats)") +
  scale_x_discrete("Timepoint", limits = 1:4, labels = 1:4) +
scale_y_discrete("PTGI-spiritual", limits = 1:5, labels = 1:5) +
theme_minimal()
