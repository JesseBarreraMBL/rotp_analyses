...
setwd("/Users/rachaelyielder/Library/CloudStorage/Box-Box/COVID Intervention + Physio/Analyses/ROTP Non-Physio Preprocessing/Step 1 - Merge, Recode, Deidentify/Raw Data - Non-Physio")
...
t1_raw_non_physio <- t1_raw_non_physio %>% 
  select(-c(Status, IPAddress, Progress, RecordedDate, ResponseId, RecipientLastName,
...
## NOTE: MAKE SURE WE HAVE TIMEZONES CORRECT, currently all in MST
...
t2_raw_non_physio_int <- t2_raw_non_physio_int %>% 
  filter((mdy_hm(StartDate) >= mdy_hm("11/18/22 12:28")) & 
           (mdy_hm(EndDate) <= mdy_hm("12/7/22 01:00"))) %>% 
    filter(!is.na(RecipientEmail)) %>% 
     filter(!(RecipientEmail %in% c("zoehuml@stanford.edu", "crum@stanford.edu",  
                              "aliajcrum@gmail.com", "zoehuml1@gmail.com", 
                              "barreraj@stanford.edu", "klouis@stanford.edu", 
...
t3_raw_non_physio <- t3_raw_non_physio %>% 
  filter((mdy_hm(StartDate) >= mdy_hm("12/15/22 15:00")) & 
...
# Make list of participants who completed T1 survey

t1_completed <- t1_raw_non_physio %>% 
  filter(Finished == "1") %>% 
  select(RecipientEmail)

# Filter out participants not included in t1_completed
...
# Make a list of participants who meet 1/3 median time criterion

t1_meets_duration <- t1_raw_non_physio %>% 
  filter(as.numeric(Duration..in.seconds.) >= (median(as.numeric(t1_raw_non_physio$Duration..in.seconds.))/3)) %>% 
  select(RecipientEmail)

# Filter out participants who don't meet criteria

t1_raw_non_physio <- t1_raw_non_physio %>% 
  filter(RecipientEmail %in% t1_meets_duration$RecipientEmail)
...
non_physio_merged_raw <- non_physio_merged_raw %>%
mutate(participant_retention_non_physio = case_when(Finished_t8_non_physio == '1' & Finished_t7_non_physio == '1' &    Finished_t6_non_physio == '1'  ~ "7", #Completed ALL timepoints
                                                    Finished_t8_non_physio == '0' & Finished_t7_non_physio == '1' & Finished_t6_non_physio == '0' ~ "6", #DNC T6, returned for T7, lost at T8
                                                    Finished_t8_non_physio == '1' & Finished_t7_non_physio == '0' & Finished_t6_non_physio == '1' ~ "5", #DNC T7, returned for T8
                                                    Finished_t8_non_physio == '1' & Finished_t7_non_physio == '0' & Finished_t6_non_physio == '0' ~ "4", #DNC T6 or T7, returned for T8
                                                    Finished_t8_non_physio == '1' & Finished_t7_non_physio == '1' & Finished_t6_non_physio == '0' ~ "3", #DNC T6, returned for T7 & T8
                                                    Finished_t8_non_physio == '0' & Finished_t7_non_physio == '1' & Finished_t6_non_physio == '1' ~ "2", #Lost at T8
                                                    Finished_t8_non_physio == '0' & Finished_t7_non_physio == '0' & Finished_t6_non_physio == '1' ~ "1", #Lost at T7
                                                    Finished_t8_non_physio == '0' & Finished_t7_non_physio == '0' & Finished_t6_non_physio == '0' ~ "0" )) #Lost at T6
