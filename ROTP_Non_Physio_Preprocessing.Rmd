...
# Remove participants who may accidentally have been 
...
randomized_int <- t2_raw_non_physio_int %>% 
  select(RecipientEmail, StartDate) %>% 
  mutate(c)

randomized_con <- t2_raw_non_physio_con %>% 
  select(RecipientEmail, StartDate)

t2_raw_non_physio <- bind_rows(randomized_int, randomized_con) %>% 
  arrange(mdy_hm(StartDate)) %>% 
  distinct(RecipientEmail, .keep_all = TRUE)
...
## Merge timepoints into single wide dataframe
...
t2_merged_duplicates <- t2_raw_non_physio$RecipientEmail[duplicated(t2_raw_non_physio$RecipientEmail)]

# Merge T1 and T2 dataframes using left_join (retains all rows in T1 and matching rows in T2)

t1_t2_non_physio <- left_join(t1_raw_non_physio, t2_raw_non_physio, by = "RecipientEmail")

t1_t2_test <- duplicated(t1_t2_non_physio$RecipientEmail)
